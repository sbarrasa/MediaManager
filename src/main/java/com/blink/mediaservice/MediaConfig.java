...
public class MediaConfig {
	...
	public MediaS3 s3(@Value("${aws.access.key.id}") String accessKey,
			@Value("${aws.secret.access.key}") String secretKey, @Value("${aws.s3.region}") String region,
			@Value("${aws.s3.bucket.name}") String bucket, @Value("${com.blink.mediamanager.path}") String path) {

		return new MediaS3().setAccessKey(accessKey).setSecretKey(secretKey).setBucket(bucket).setRegion(region)
				.setPath(path);

	}

	public MediaTemplate newMediaTemplate(String className, String path) {
		try {
	...
	public MediaTemplate mediaTemplate(@Value("${com.blink.mediamanager.class}") String className,
		...
		return newMediaTemplate(className, path);
	}
